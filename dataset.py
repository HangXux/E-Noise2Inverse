...
class Noise2InverseDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # normalize
        input_min = torch.min(inp)
        input_max = torch.max(inp)
        target_min = torch.min(tgt)
        target_max = torch.max(tgt)
        inp = torch.div(inp - input_min, input_max - input_min)
        tgt = torch.div(tgt - target_min, target_max - target_min)
